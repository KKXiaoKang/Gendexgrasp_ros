...
namespace HighlyDynamic
{
    ...
    std::map<std::string, motor_config> g_motor_name_map = {
                ...
                {"ruiwo", {BIT_17_36, CK_MC, RUIWO_C2T, RUIWO}},
    ...
    int JSONConfigReader::getLegJointCount()
    {   
        // FIXME: 当前 NUM_JOINT 不包含头部关节， 因此减去 0
        // 当 NUM_JOINT 包含头部关节时，应当减去头部关节数量
        int num_joint_without_head = NUM_JOINT - 0;
        return num_joint_without_head - NUM_ARM_JOINT;
    }
    bool JSONConfigReader::flagHalfUpBody() 
    {
        // try {
        //     return getValue<bool>("HALF_UP_BODY");
        // }
