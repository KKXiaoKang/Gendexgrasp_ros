...
void Environment::SyncConfig() {
    ...
    // fix the ruiwo to joint name sequence error
    auto swap_last_two_if_needed = [&](std::vector<std::string>& joint_names) {
        if (joint_names.size() >= 2 && 
            joint_names[joint_names.size() - 2] == "head_pitch" && 
            joint_names.back() == "head_yaw") {
            std::swap(joint_names[joint_names.size() - 2], joint_names.back());
            std::cout << "[Environment] SyncConfig, Swap head_pitch and head_yaw seq in ruiwo 2 joint names.\n";
        }
        config_data["ruiwo_2_joint_name"] = joint_names;
    };
    // Check if the key exists before accessing it
    if (config_data.contains("ruiwo_2_joint_name")) {
        auto ruiwo_2_joint_name = config_data["ruiwo_2_joint_name"].get<std::vector<std::string>>();
        swap_last_two_if_needed(ruiwo_2_joint_name);
    } 
